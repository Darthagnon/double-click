...
~LButton::
   ...
   else if ( IsDblClick() && (hWnd := WinActive("ahk_class WorkerW"))) ; If on desktop, double-click to toggle hide desktop icons
      HideIcons()
   Return
   
; "Is this a double-click?" logic
IsDblClick() {
   Return A_PriorHotkey = A_ThisHotkey && A_TimeSincePriorHotkey < DllCall("GetDoubleClickTime") ; Prevent triple-clicking
}

; Hide desktop icons logic   
HideIcons() {
   ControlGet, HWND, Hwnd,, SysListView321, ahk_class Progman
   if HWND = ControlGet, HWND, Hwnd,, SysListView321, ahk_class WorkerW
